...
import { PILGRIM_WS_URL } from '../../../config/pilgrim.config'
import { monitoringAPI } from '../../../services/adminService'
...
const DatabaseMonitor = () => {
  ...
  const fetchHttpMonitoring = async () => {
    try {
      const data = await monitoringAPI.getSystemStatus()
      if (data) {
        setMonitoringData(data.data || data)
        setIsConnected(true)
        setError(null)
      }
    } catch (err) {
      console.warn('Fallback HTTP monitoring status failed:', err)
      setError('No se pudo conectar al servicio de monitoreo')
    }
  }
  ...
  const connectWebSocket = () => {
    try {
      const wsUrl = `${PILGRIM_WS_URL}/monitoring/ws`
      ...
      wsRef.current.onerror = err => {
        console.warn('⚠️ WebSocket no disponible, activando sondeo HTTP fallback')
      ...
      wsRef.current.onclose = () => {
        setIsConnected(false)
        if (autoRefresh && !pollTimerRef.current) {
      ...
      console.warn('⚠️ Error creando WebSocket, usando fallback HTTP:', err)
      fetchHttpMonitoring()
    }
  }
